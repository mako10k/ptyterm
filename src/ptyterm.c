  ...
  switch (pid = fork ())
      ...
      else if (opt_cols > 0 || opt_lines > 0)
	{
	  if (opt_cols <= 0)
	    {
	      fprintf (stderr,
		       "cols is not set with lines for notty inputs\n");
	      exit (EXIT_FAILURE);
	    }
	  if (opt_lines <= 0)
	    {
	      fprintf (stderr,
		       "lines is not set with cols for notty inputs\n");
	      exit (EXIT_FAILURE);
	    }
      ...
      for (;;)
	{
	  ...
	  if (FD_ISSET (STDIN_FILENO, &rfds))
	    {
	      ssize_t siz =
		read (STDIN_FILENO, buf2 + siz2, sizeof (buf2) - siz2);
	      if (siz == -1)
		{
		  perror ("read");
		  exit (EXIT_FAILURE);
